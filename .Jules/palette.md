...
## 2026-04-10 - Focus Management in Dismissible Widgets
**Learning:** Floating, dismissible widgets (like the Giles AI helper) must dynamically manage `aria-expanded` and strictly enforce focus state continuity. Simply closing the widget leaves keyboard focus in a "dead zone", causing disorientation for screen reader users.
**Action:** When implementing any toggleable widget, use `focus()` to pass the interaction context to the close button upon opening, and return it to the trigger button upon closure, ensuring a complete and seamless accessible user journey.
