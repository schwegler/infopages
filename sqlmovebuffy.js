...
export function createDeaChart(containerId, data) {
     const el = document.getElementById(containerId);
    el.innerHTML = `
        <div class="space-y-3">
            ${Object.entries(data).filter(([key]) => key !== 'Insight').map(([key, value]) => `
        ...
        <p class="text-center mt-4 sub-heading">${data.Insight ? data.Insight.text : ''}</p>
    `;
}
...
export function initPage() {
    ...
    function toggleGiles() {
        ...
        gilesToggleBtn.setAttribute('aria-expanded', !isHidden);

        if (!isHidden && gilesChatLog.children.length === 0) {
